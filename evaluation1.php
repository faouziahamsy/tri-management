...
$stmt = $conn->query("SELECT e.*,concat(r.firstname,' ',r.lastname) as name ,intitule
					 FROM evaluation1 e  JOIN users r 
...
<body onload="CreateTable(fetchdata)">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="mb-3">
					...
					<select type="text" class="form-control" id="type" value=" " >
					<option value="">--- Choisir un type ---</option>
					<option value="Controle">Controle</option>
					<option value="Examen">Examen</option>
					<option value="Examen de tp">Examen de tp</option>
				</select>
				</div>
			...
			for (let i = 0; i < lines.length; i++) {
				...
				seances.forEach(seance => {
					if (td.dataset.jour == seance.jour && td.dataset.heuredebut==seance.heuredebut && td.dataset.heurefin==seance.heurefin) {
						...
						td.innerHTML="<p><b>"+seance.intitule+"</b>("+seance.name+")"+"</br>salle:"+seance.salle+"</br>"+seance.type+"   </p>"
					}
	...
	<script>
			...
			$.ajax({
				url: 'ajax.php?action=add_evaluation1',
				method: 'POST',
				data: {
