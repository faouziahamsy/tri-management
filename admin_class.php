...
Class Action {
	...
	function delete_module(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM module where module_id = ".$module_id);
		if($delete)
			return 1;
	}

	function delete_module1(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM module1 where module_id = ".$module_id);
		if($delete)
			return 1;
	}

	function delete_sujet(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM cellule where id_sujet = ".$id_sujet);
		if($delete)
			return 1;
	}
	...
	function save_module(){
		...
		if(empty($module_id)){
			$save = $this->db->query("INSERT INTO module set $data");
			if($save){
				return 1;
			}
		}else{
			$save = $this->db->query("UPDATE module set $data where module_id = $module_id");
			if($save){
				return 2;
			}
		}
	...
	function save_module1(){
		...
		if(empty($module_id)){
			$save = $this->db->query("INSERT INTO module1 set $data");
			if($save){
				return 1;
			}
		}else{
			$save = $this->db->query("UPDATE module1 set $data where module_id = $module_id");
			if($save){
				return 2;
			}
		}
	...
	function save_cellule(){
		...
		if(empty($cellule_id)){
			$save = $this->db->query("INSERT INTO cellule set $data");
		}else{
			$save = $this->db->query("UPDATE cellule set $data where id_cellule = $cellule_id ");
		}
		if($save){
			return 1;
		}
