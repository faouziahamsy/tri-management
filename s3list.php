	...
	<div class="card card-outline card-success">
		...
		<?php if($_SESSION['login_type'] ==1 ||$_SESSION['login_type'] ==4): ?>
			<div class="card-tools">
				<a class="btn btn-block btn-sm btn-danger btn-flat border-black" href="./index.php?page=addnewmodule&semestr=3"><i class="fa fa-plus"></i> Ajouter un module dans S3</a>
			</div>
            <?php endif; ?>
		</div>
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT * FROM module,users,semestre WHERE module.responsable = users.id AND semestre.id_semestre=module.id_semestre AND code_semestre='S3' ");
					while($row= $qry->fetch_assoc()):
						$trans = get_html_translation_table(HTML_ENTITIES,ENT_QUOTES);
						unset($trans["\""], $trans["<"], $trans[">"], $trans["<h2"]);
					 	$tprog = $conn->query("SELECT * FROM module where module_id = {$row['module_id']}")->num_rows;
					?>
					<tr>
						...
						<td>
							...
							<button type="button" class="btn btn-danger  btn-block delete_module " href="javascript:void(0)" data-module_id="<?php echo $row['module_id'] ?>" >Supprimer </button>
						</td>
					<?php endif; ?>
					</tr>	
				<?php endwhile; ?>
...
<script>
	...
	_conf("Etes vous sur de vouloir supprimer ce module?","delete_module",[$(this).attr('data-module_id')])
	})
	})
