	...
	<div class="card card-outline card-success">
		<div class="card-header">
			...
			<div class="card-tools">
				<a class="btn btn-block btn-sm btn-default btn-flat border-primary" href="./index.php?page=addnewmodule"><i class="fa fa-plus"></i> Add New Module</a>
			</div>
            <?php endif; ?>
		</div>
		<div class="card-body">
			<table class="table tabe-hover table-condensed" id="list">
				...
				<tbody>
					...
					while($row= $qry->fetch_assoc()):
						...
						if($row['status'] == 0 && strtotime(date('Y-m-d')) >= strtotime($row['RESPONSABLE'])):
						if($prod  > 0  || $cprog > 0)
		                  $row['status'] = 2;
		                else
		                  $row['status'] = 1;
					...
					<tr>
						...
						<td><b><?php echo date("M d, Y",strtotime($row['RESPONSABLE'])) ?></b></td>
